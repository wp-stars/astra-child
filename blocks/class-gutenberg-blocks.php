...
namespace Limesoda\Astra_Child\Blocks;
...
if (!class_exists('Gutenberg_Blocks')) {
	...
	class Gutenberg_Blocks {
		 ...
		 * NOTE: Registers only block assets registered by this class (<theme>/blocks)
		 ...
		 *
		 * Block assets must be placed inside <theme>/blocks/<slug>/
		 ...
		 *
		 * options for <theme>/blocks/<slug>/index.php return array:
		 * 'slug': name of block
		...
		public function set_blocks() {
			self::$blocks = array_map(
				...
				glob(THEME_DIR . 'blocks/*', GLOB_ONLYDIR)
			);
		}
		...
		public function register_block_assets() {
			foreach (self::$blocks as $block) {
				$group = isset($block['group']) ? $block['group'] : 'ls';
				if ($group === 'ls') {
					$block_dirname = $group . '-' . $block['slug'];
					$block_uri = trailingslashit(THEME_URI . 'blocks/' . $block_dirname);
					$block_dir = trailingslashit(THEME_DIR . 'blocks/' . $block_dirname);
						...
						if (file_exists($block_dir . $asset[1] . '.' . $asset[2])) {
							...
							if ($asset[2] === 'js') {
								...
								wp_register_style(
									...
									$block_uri . $asset[1] . '.' . $asset[2],
									[],
									filemtime(THEME_DIR . 'blocks/' . $block_dirname . '/index.css')
								);
							}
						}
					}
					...
					if (!WP_Block_Type_Registry::get_instance()->is_registered(trailingslashit($group) . $block['slug'])) {
						...
						if (file_exists($block_dir . 'frontend.css')) {
							wp_enqueue_style(
								...
								THEME_URI . 'blocks/' . $block_dirname . '/frontend.css',
								[],
								filemtime(THEME_DIR . 'blocks/' . $block_dirname . '/frontend.css')
							);
						}
					}
				}
		...
		public function enqueue_block_override_frontend_assets() {
			foreach (self::$blocks as $block) {
				...
				if ($group !== 'ls') {
					$block_dirname = $group . '-' . $block['slug'];
					$has_block = has_block(trailingslashit($group) . $block['slug']) || in_array(trailingslashit($group) . $block['slug'], self::$additional_block_dependencies) || $block['loadGlobally'];

					if ($has_block || $block_dirname === 'core-group' || $block_dirname === 'core-paragraph') {
						$asset_file = include THEME_DIR . '/blocks/' . $block_dirname . '/index.asset.php';

						wp_enqueue_script(
							'block-frontend-script-' . $block_dirname,
							THEME_URI . 'blocks/' . $block_dirname . '/index.js',
							$asset_file['dependencies'],
							$asset_file['version'],
							true
						);
						...
						wp_enqueue_style(
							...
							THEME_URI . 'blocks/' . $block_dirname . '/index.css',
							[],
							filemtime(THEME_DIR . 'blocks/' . $block_dirname . '/index.css'),
						);
					}
		...
		public function enqueue_block_override_editor_assets() {
			foreach (self::$blocks as $block) {
				...
				if ($group !== 'ls') {
					...
					$asset_file = include THEME_DIR . '/blocks/' . $dir . '/index.asset.php';

					wp_enqueue_script(
						'block-frontend-script-' . $dir,
						THEME_URI . 'blocks/' . $dir . '/index.js',
						$asset_file['dependencies'],
						$asset_file['version'],
						true
					);
					...
					wp_enqueue_style(
						...
						THEME_URI . 'blocks/' . $dir . '/index.css',
						[],
						filemtime(THEME_DIR . 'blocks/' . $dir . '/index.css')
					);
					wp_enqueue_style(
