...
namespace Limesoda\Astra_Child\Cover\LS_Cover_Slider;
...
add_action('wp', function () {
	...
	if ($cover_slider[0] === 'true') {
		add_action('wp_enqueue_scripts', '\Limesoda\Astra_Child\Cover\LS_Cover_Slider\enqueue_cover_assets');
		add_action('astra_content_before', '\Limesoda\Astra_Child\Cover\LS_Cover_Slider\add_cover_slider_markup');
	}
});
	...
	if ($content_type === 'custom') {
		...
		if ($posts) {
			foreach ($posts as $post) {
				...
				$post_args['date'] = date_i18n('M Y', strtotime(get_the_date('M Y', $post_id)));
