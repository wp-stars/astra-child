...
namespace Limesoda\Astra_Child\News\Detailpage;
...
add_action('wp_enqueue_scripts', '\Limesoda\Astra_Child\News\Detailpage\enqueue_post_assets');
...
add_action('wp', '\Limesoda\Astra_Child\News\Detailpage\disable_post_title');
...
function disable_the_content($the_content) {
	...
	if (!in_array(get_post_type(), $post_types, true)) {
		...
		$html .= '<div class="wp-block-button"><a id="ls-linkedin-share-button" class="wp-icons icon-linkedin"></a></div>';
		$html .= '</div>';
...
add_filter('the_content', '\Limesoda\Astra_Child\News\Detailpage\disable_the_content');
