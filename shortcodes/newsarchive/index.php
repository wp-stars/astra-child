...
if (!class_exists('Newsarchive')) {
	...
	class Newsarchive {
		...
		public function shortcode_markup($attributes = []): string {
			...
			if ($query->have_posts()) {
				while ($query->have_posts()) {
					...
					$html .= $this->single_post_card($title, $date, $category, $category_slug, $excerpt, $link, $media, $cat_icon_url, $cat_icon_alt, $cat_icon_title);
				}
		 ...
		 */
		public function single_post_card($title, $date, $category, $category_slug, $excerpt, $link, $media, $cat_icon_url, $cat_icon_alt, $cat_icon_title): string {
			$html = '<div class="ls-post-card">';
			if (isset($media) && isset($category_slug)) {
			...
			$html .= '<div class="ls-post-card__metadata"><p class="ls-post-card__date">' . $date . ' | </p><p class="ls-post-card__category">' . $category . '</p></div>';
